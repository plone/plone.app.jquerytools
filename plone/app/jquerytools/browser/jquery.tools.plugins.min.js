...
/*
 ...
 Date: 2011-10-26 11:02 
*/
(function(b){var i=b.tools.scrollable;i.autoscroll={conf:{autoplay:true,interval:3E3,autopause:true}};b.fn.autoscroll=function(c){if(typeof c=="number")c={interval:c};var d=b.extend({},i.autoscroll.conf,c),j;this.each(function(){function f(){e=setTimeout(function(){a.next()},d.interval)}var a=b(this).data("scrollable"),g=a.getRoot(),e,h=false;if(a)j=a;a.play=function(){if(!e){h=false;g.bind("onSeek",f);f()}};a.pause=function(){e=clearTimeout(e);g.unbind("onSeek",f)};a.resume=function(){h||a.play()};
a.stop=function(){h=true;a.pause()};d.autopause&&g.add(a.getNaviButtons()).hover(a.pause,a.resume);d.autoplay&&a.play()});return d.api?j:this}})(jQuery);
...
"/>").click(function(f){h(e(this),a,f)});a===0&&c.addClass(k);b.indexed&&c.text(a+1);b.idPrefix&&c.attr("id",b.idPrefix+a);return c.appendTo(j)}var d=e(this).data("scrollable"),j=b.navi.jquery?b.navi:p(d.getRoot(),b.navi),q=d.getNaviButtons(),k=b.activeClass,n=b.history&&!!history.pushState,l=d.getConf().size;if(d)g=d;d.getNaviButtons=function(){return q.add(j)};if(n){history.pushState({i:0});e(window).bind("popstate",function(a){(a=a.originalEvent.state)&&d.seekTo(a.i)})}i().length?i().each(function(a){e(this).click(function(c){h(e(this),
...
j);j();return b},pause:function(){if(!i)return b;var f=c.Event("onBeforePause");h.trigger(f);if(f.isDefaultPrevented())return b;i=clearTimeout(i);h.trigger("onPause");h.unbind("onClick",j);return b},resume:function(){k||b.play()},stop:function(){b.pause();k=true}});c.each("onBeforePlay,onPlay,onBeforePause,onPause".split(","),function(f,e){c.isFunction(a[e])&&c(b).bind(e,a[e]);b[e]=function(r){return c(b).bind(e,r)}});a.autopause&&d.getTabs().add(o).add(l).add(d.getPanes()).hover(b.pause,b.resume);
a.autoplay&&b.play();a.clickable&&d.getPanes().click(function(){d.next()});if(!d.getConf().rotate){var m=a.disabledClass;d.getIndex()||l.addClass(m);d.onBeforeClick(function(f,e){l.toggleClass(m,!e);o.toggleClass(m,e==d.getTabs().length-1)})}}var p;p=c.tools.tabs.slideshow={conf:{next:".forward",prev:".backward",disabledClass:"disabled",autoplay:false,autopause:true,interval:3E3,clickable:true,api:false}};c.fn.slideshow=function(g){var a=this.data("slideshow");if(a)return a;g=c.extend({},p.conf,g);
this.each(function(){a=new q(c(this),g);c(this).data("slideshow",a)});return g.api?a:this}})(jQuery);
...
'<param name="'+d+'" value="'+a[d]+'" />';a="";if(b){for(var h in b)if(b[h]){d=b[h];a+=h+"="+encodeURIComponent(/function|object/.test(typeof d)?e.asString(d):d)+"&"}a=a.slice(0,-1);c+='<param name="flashvars" value=\''+a+"' />"}c+="</object>";return c},isSupported:function(a){return g[0]>a[0]||g[0]==a[0]&&g[1]>=a[1]}}),g=e.getVersion();if(n){jQuery.tools=jQuery.tools||{version:"v1.2.6"};jQuery.tools.flashembed={conf:j};jQuery.fn.flashembed=function(a,b){return this.each(function(){jQuery(this).data("flashembed",
flashembed(this,a,b))})}}})();
